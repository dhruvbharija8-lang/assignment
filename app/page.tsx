...
import { buildTimelineMap } from '@/src/utils/parseVideos';
import { requestVideoList, getVideoList, requestClipAndWait } from '@/src/utils/api';
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (currentIndex !== null && currentIndex + 1 < clips.length && !loading) {
      const nextFilename = clips[currentIndex + 1].forward || clips[currentIndex + 1].inward;
      requestClipAndWait(nextFilename).catch(() => {
        // Silently ignore prefetch errors
      });
    }
  }, [currentIndex, clips, loading]);
